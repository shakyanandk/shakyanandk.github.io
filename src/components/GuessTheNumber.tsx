...
const GuessTheNumber = () => {
  ...
  const handleGuess = () => {
    ...
    if (numGuess === targetNumber) {
      setMessage(
        `🎉 Correct! You guessed it in ${newAttempts} ${
  ...
  return (
    <>
      <Confetti active={gameStatus === "won"} type="fall"/>
          ...
          {gameStatus === "playing" ? (
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 bg-background border border-border/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <Button
                onClick={handleGuess}
                className="px-6 sm:px-4 flex-shrink-0"
              >
                Guess
              </Button>
            </div>
